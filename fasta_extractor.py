...
def user_input():
    ...
    the -i flag indicates that ~/Documents/resutls is the path to the directory
    ...
    case, results will be the input folder and will contain the SW0001 and
    SW0002 assembly.fasta results as follows:
    ...
    every time it finds a header, i. e. a '>' symbol. If the EOF is reached,
    the script will continues with the next SW0002 subfolder. If the user
    doesn't provide an output path, the new fasta files will be saved in the
    ...
    An example of a possible results if the user doesn't provide an
    output path:
...
def get_file_paths(file_name, input_folder):
    """
    Gets the paths of files with the same name that are contained in primary
    subdirectories.

    Iterates over subdirectories of a given directory (input_folder) looking
    for files with the same name (file_name). Returns the paths to these
    files.
    ...
    For example, if you have the hypothetical three:
    ...
    ["~/My_assemblies/SW0001/assembly.fasta",
     "~/My_assemblies/SW0002/assembly.fasta"]
    """
...
def fasta_extractor(input_file, output_folder=None):
    """
    Parse a fasta file to extract the fasta sequences that the fasta file
    contains.

    The extracted fasta sequences are saved into independent files. If the user
    don't specify the output_folder, the new fasta files are outputed in the
    same directory where the input_file is located. The extracted fasta
    sequences are named according to the name of the directory that contains
    the input_file.
